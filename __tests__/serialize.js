...
const roundTrip = (s, parserOptions) => htmlparser.serialize(htmlparser.parse(s, parserOptions));
...
test('recognizeSelfClosing', () => {
  expect(roundTrip('<x1/><x2/>', { recognizeSelfClosing: true })).toBe('<x1></x1><x2></x2>');
});

test('misc HTML', () => {
  expect(roundTrip('<div></div   ><p>&lt;</p><!--<x>--><input    type=checkbox checked/>')).toBe(
